#!/usr/bin/env python3
"""One-time migration: extract template constants into asset files.

Phase 2 of the robustness refactor plan (docs/robustness-refactor-plan-
2026-09-12.md). Reads every constant from the CURRENT templates.py /
templates_domain.py modules and writes the exact strings to
src/reportforge/templates/_assets/<family>/<file>, then generates the
loader package __init__.py that re-exposes the same attribute names.

Byte-exactness contract: the value stored in each asset file equals the
old constant's value exactly (str → file bytes, UTF-8, no newline
normalization), so scaffolds reproduce byte-identical projects.

Run from the repo root BEFORE deleting templates.py/templates_domain.py:
    .venv/bin/python scripts/extract_template_assets.py
"""
...
PACKAGE = REPO / "src" / "reportforge" / "templates"
ASSETS = PACKAGE / "_assets"

# attribute name → asset path (relative to _assets/)
SPEC: dict[str, str] = {
    # shared scaffolding
...
LOADER_TEMPLATE = '''"""Template assets for reportforge.scaffold (loader package).
...
def main() -> int:
    import reportforge.templates as old_templates  # the module, pre-deletion
    import reportforge.templates_domain as old_domain

    PACKAGE.mkdir(exist_ok=True)
    written = 0
    for name, rel in SPEC.items():
        value = getattr(old_templates, name)
        assert isinstance(value, str), name
        path = ASSETS / rel
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(value, encoding="utf-8", newline="")
        written += 1
    for attr, slug in DOMAIN_SLUGS.items():
        value = getattr(old_domain, attr)
        assert isinstance(value, str), attr
        path = ASSETS / "domains" / f"{slug}.qmd"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(value, encoding="utf-8", newline="")
        written += 1
    ...
    loader = LOADER_TEMPLATE.format(
        ...
        domain_slugs=(
            "{\n"
            + "".join(f'    "{k}": "{v}",\n' for k, v in DOMAIN_SLUGS.items())
            + "}"
        ),
    )
    (PACKAGE / "__init__.py").write_text(loader, encoding="utf-8", newline="")
    print(f"wrote {written} asset files + loader __init__.py under {PACKAGE}")
    return 0
