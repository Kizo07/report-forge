...
#let ledger_dark(
  ...
  metrics: (), verdict: none, key-points: (), scenarios: (),
  organization: none, eyebrow: none,
  title-layout: "hero", accent: "#e3ac55", confidential-mark: none,
  cols: 1, margin: (x: 0.82in, top: 0.72in, bottom: 0.9in),
  paper: "us-letter", lang: "en", region: "US",
  font: none, fontsize: 10.5pt, mathfont: none, codefont: none,
  linestretch: 1.15, sectionnumbering: none, linkcolor: none,
  ...
  set page(
    ...
    header: context {
      ...
      if num >= 2 {
        block(width: 100%)[
          #set text(size: 7.8pt, fill: muted, font: "IBM Plex Mono")
          #grid(columns: (1fr, auto), align: horizon)[
            #if organization != none [#upper[#organization]] else if eyebrow != none [#upper[#eyebrow]] else [LEDGER REPORT]
          ][#date]
          #line(length: 100%, stroke: 0.55pt + hairline)
        ]
      }
  ...
  show table: set table(
    stroke: 0.5pt + hairline,
    inset: (x: 6pt, y: 5pt),
    fill: (x, y) => if y == 0 { rgb("#101a29") } else if calc.rem(y, 2) == 0 { rgb("#080d16") } else { panel },
  )
  show table: set text(size: 8.7pt)
  show table.cell.where(y: 0): set text(weight: "bold", fill: ink)
  show table.cell.where(x: 0): set text(weight: "bold")
    ...
    if title-layout == "minimal" {
      ...
      block(width: 100%)[
        #set par(justify: false)
          ...
          line(length: 26pt, stroke: 1.4pt + accent-color)
          v(0.35em)
          set text(font: "IBM Plex Mono", size: 8pt, weight: "medium", fill: accent-color)
          upper[#eyebrow]
          v(0.5em)
        }
        #set text(font: "Space Grotesk", size: 24pt, weight: "bold", fill: ink, hyphenate: false)
      ...
      block(
        ...
        #set par(justify: false)
          ...
          line(length: 26pt, stroke: 1.4pt + accent-color)
          v(0.4em)
          set text(font: "IBM Plex Mono", size: 8.5pt, weight: "medium", fill: accent-color)
          upper[#eyebrow]
          v(0.7em)
        }
        #set text(font: "Space Grotesk", size: 22pt, weight: "bold", fill: ink, hyphenate: false)
  ...
  if metrics != () and metrics.len() > 0 {
    ...
    grid(
      ...
      ..metrics.map(metric => block(
        ...
        #metric.value
        #linebreak()
        #set text(font: "IBM Plex Mono", size: 7.7pt, fill: muted)
        #upper[#metric.label]
      ])
    )
    v(0.55em)
  ...
  if verdict != none {
    block(
      ...
      #set text(font: "IBM Plex Mono", size: 7.7pt, weight: "medium", fill: accent-color)
      CONVICTION CALL
      #linebreak()
      #set text(size: 9.5pt, weight: "bold", fill: ink)
      #verdict
...
blockquote {
  ...
  padding: 1rem 1.25rem;
  border: 1px solid var(--rf-line);
  border-left: 5px solid var(--rf-accent, #e3ac55);
  border-radius: 0 12px 12px 0;
  background: var(--rf-panel);
  color: var(--rf-ink);
  font-style: normal;
...
#let ledger_light(
  ...
  show table: set table(
    stroke: 0.5pt + hairline,
    inset: (x: 6pt, y: 5pt),
    fill: (x, y) => if y == 0 { rgb("#d3dde6") } else if calc.rem(y, 2) == 0 { rgb("#d8e1e9") } else { panel },
  )
  show table: set text(size: 8.7pt)
  show table.cell.where(y: 0): set text(weight: "bold", fill: ink)
  show table.cell.where(x: 0): set text(weight: "bold")
